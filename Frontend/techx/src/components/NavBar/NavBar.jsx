          ...
          showLogin && 
              <div className='flex absolute z-50  w-screen h-screen sm:w-[208vh] sm:h-[100vh] justify-center items-center opacity-85 top-0  bg-black '>
                  <div className='flex flex-col gap-4 bg-white p-5 rounded-md w-[40vh] min-h-80 relative'>
                    <span className='text-2xl text-black font-bold text-center'>{status==='LOGIN'?'Welcome Back':'Create an Account'}</span>
                    <IoIosCloseCircleOutline className='absolute right-2 top-2 text-[#25AEFF] cursor-pointer hover:text-[#376f8f]' size={22} onClick={()=>(setShowLogin(false))} />
                    <div className='flex flex-col gap-2 '>
                    
                    <span className='flex flex-col gap-1'>
                      <label className='text-xs text-black font-medium'>User Name</label>
                      <input type='text' className='flex text-white px-3 py-2 w-full bg-[#383838] rounded-md text-xs' />
                    </span>
                    <span className='flex flex-col gap-1'>
                      <label className='text-xs text-black font-medium'>Email</label>
                      <input type='email' className='flex text-white px-3 py-2 w-full bg-[#383838] rounded-md text-xs' />
                    </span>
                    {status==='LOGIN'?'':<span className='flex flex-col gap-1'>
                      <label className='text-xs text-black font-medium'>Mobile Number</label>
                      <input type='text' className='flex text-white px-3 py-2 w-full bg-[#383838] rounded-md text-xs'/>
                    </span>}
                    <span className='flex flex-col gap-1'>
                      <label className='text-xs text-black font-medium'>Password</label>
                      <input type='password' className='flex text-white px-3 py-2 w-full bg-[#383838] rounded-md text-xs' />
                    </span>

                    <button className='flex justify-center items-center px-2 mt-2 bg-[#25AEFF] rounded-md py-2 font-medium text-sm transition-all hover:bg-[#35769e] cursor-pointer'>{status==='LOGIN'?"SIGN IN":"SIGN UP"}</button>
                    
                    {status==='LOGIN'?(<span className=' flex flex-row gap-1 text-xs justify-center text-black font-medium'>Don't have an account?<span className='text-[#25AEFF] cursor-pointer' onClick={()=>(setStatus('REGISTER'))}>Register</span></span>):
                    (<span className=' flex flex-row gap-1 text-xs justify-center text-black font-medium'>All ready have an account?<span className='text-[#25AEFF] cursor-pointer' onClick={()=>(setStatus('LOGIN'))}>Login</span></span>)}
                    </div>
